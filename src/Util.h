...
/**
 * reates a path from an url, for example:
 ...
 * arguments separated into options and positional arguments.
 *
...
public:
	template <class T>
	ArgumentParseEx(T&& arg)
		: std::runtime_error(std::forward<T>(arg)){};
};
